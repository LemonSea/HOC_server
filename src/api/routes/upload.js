...
module.exports = (app) => {
  ...
  route.post('/company/pic', function (req, res) {
    ...
    var targetDir = path.join(__dirname, '../../public/imgs/company/pic');
    // 检查目标目录，不存在则创建
    fs.access(targetDir, function (err) {
      if (err) {
        fs.mkdirSync(targetDir);
      ...
      form.parse(req, function (err, fields, files) {
        ...
        keys.forEach(function (key) {
          ...
          if (('.jpg.jpeg.png.gif').indexOf(fileExt.toLowerCase()) === -1) {
            ...
            // 文件的Url（相对路径）
            // filesUrl.push('/staff/pic/' + fileName)
            filesUrl = '/imgs/company/pic/' + fileName;
          }
        });
        const data = {
          name: fileName,
  ...
  route.post('/staff/avatar', function (req, res) {
    ...
    var targetDir = path.join(__dirname, '../../public/imgs/staff/avatar');
    // 检查目标目录，不存在则创建
    fs.access(targetDir, function (err) {
      if (err) {
        fs.mkdirSync(targetDir);
      ...
      form.parse(req, function (err, fields, files) {
        ...
        keys.forEach(function (key) {
          ...
          if (('.jpg.jpeg.png.gif').indexOf(fileExt.toLowerCase()) === -1) {
            ...
            // 文件的Url（相对路径）
            // filesUrl.push('/staff/pic/' + fileName)
            filesUrl = '/imgs/staff/avatar/' + fileName;
          }
        });
        const data = {
          name: fileName,
